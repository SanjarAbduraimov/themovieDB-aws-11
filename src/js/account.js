...
import { fetchMovieFavorityDel , fatchMovieRatingDel} from "../api";
import Type, {
  status,
  credits,
  sortBy,
...
export function displayFavoriteTiv(data = []) {
  const authorMenuNode = document.querySelector(".profile__wrapper");
  authorMenuNode.innerHTML = "" 
  const profilewatchlist = document.querySelector(".no_results");
  const addedSameNav = document.querySelector(".title__profiles-page");
...
export function displayratedTiv(data = []) {
  const authorMenuNode = document.querySelector(".profile__wrapper");
  authorMenuNode.innerHTML = "" 
  const profilewatchlist = document.querySelector(".no_results");
  const addedSameNav = document.querySelector(".title__profiles-page");
  ...
  let result = "";
  data.forEach((keyword) => {
    const { backdrop_path, id, title, release_date, overview, name, rating } = keyword;
    const imgs = backdrop_path
      ? configs.baseImgURL + backdrop_path
      : configs.defaultImg + "500";
    const originName = name ? name : title;
    ...
    result += ` <div class="col"> <article class="card card__keySearc" data-id="${id}">
        ...
        <div class="card__body card__percentage">
          ...
          <div class="profile__favo">
          <li class="dropdown__item">
           <a id="rating" class="retings_accounts-mov retings_accounts-tv" data-id="${id}">
           <i class="fa-solid fa-star"></i> <p class="profile__ratingd " >Raiting </p>       </a>
          </li>
          ...
          <li class="dropdown__item">
          <a class="reatings__dels reatings__delstv" data-id="${id}">
          <i id="exxx" class="fa-solid fa-xmark" ></i> <p class="profile__ratingd " > Remove  </p>         </a>
          </li>
...
export function displayRaitingMovue(data = []) {
  const authorMenuNode = document.querySelector(".profile__wrapper");
  authorMenuNode.innerHTML = "" 
  const profilewatchlist = document.querySelector(".no_results");
  const addedSameNav = document.querySelector(".title__profiles-page");
...
export function initializeAccountEventtvs() {
  const keywordMenuNode = document.querySelector(".profile__wrapper");
  keywordMenuNode.addEventListener("click", (event) => {
    const id =
      event.target.closest(".card__img")?.dataset?.id ||
...
export function initializeRaitingsEvent() {
  const keywordMenuNode = document.querySelector(".profile__wrapper");
  keywordMenuNode.addEventListener("click", (event) => {
    const id =
      event.target.closest(".retings_accounts-mov")?.dataset?.id ||
      event.target.closest(".reatings__dels")?.dataset?.id;
    console.log(id, "bosilgan");
    if (!id) return;
    fatchMovieRatingDel(Type.movie, id)
    event.target.parentElement.parentElement.parentElement.parentElement.parentElement.remove();
  });
}

export function initializeRaitTivEvent() {
  const keywordMenuNode = document.querySelector(".profile__wrapper");
  keywordMenuNode.addEventListener("click", (event) => {
    const id =
      event.target.closest(".retings_accounts-tv")?.dataset?.id ||
      event.target.closest(".reatings__delstv")?.dataset?.id;
    console.log(id, "bosilgan");
    if (!id) return;
    fatchMovieRatingDel(Type.tv, id)
    event.target.parentElement.parentElement.parentElement.parentElement.parentElement.remove();
  });
}
